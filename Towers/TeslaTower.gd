...
export var thisScene = "res://Towers/TeslaTower.tscn"
...
export var Name = "Tesla Tower"
export var IconPath = "res://Control Assets/TeslaCoil.png"
	...
	if enemiesInRange.size() > 0 && towerIsPlaced:
		...
		if currentTarget:
			...
			if (currentTarget != null):
				AttackTimer.start()
				connect("Stun", currentTarget, "on_Stun")
...
func _on_TowerRange_body_exited(body):
	...
	if body == currentTarget:
		...
		disconnect("Stun", currentTarget, "on_Stun")
		currentTarget = null
		minDist = 999999
		AttackTimer.stop()
